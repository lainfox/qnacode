...
function login_header($title = 'Log In', $message = '', $wp_error = '') {
	...
	if ( is_multisite() ) {
		...
		$login_header_url   = __( 'http://qnacode.com/' );
		$login_header_title = __( 'QnA code' );
	}
	...
	?>
	<style type="text/css">
	.login h1 a {background:url(http://qnacode.com/qna/qa-theme/qnacode/logo.png) no-repeat 50% 50% !important;}
	.submit {height:30px;}
	/* Faded out hr */
	hr.hr {clear:both; height:1px; margin:15px 0 24px; border: 0;
		background-image: -webkit-linear-gradient(left, rgba(0,0,0,0), rgba(0,0,0,.1), rgba(0,0,0,0));
		background-image:    -moz-linear-gradient(left, rgba(0,0,0,0), rgba(0,0,0,.1), rgba(0,0,0,0));
		background-image:     -ms-linear-gradient(left, rgba(0,0,0,0), rgba(0,0,0,.1), rgba(0,0,0,0));
		background-image:      -o-linear-gradient(left, rgba(0,0,0,0), rgba(0,0,0,.1), rgba(0,0,0,0));
	}
	#wp-social-login-connect-with {display:block; padding-bottom:10px}
	#wp-social-login-connect-options > a {text-decoration:none; padding-right:4px;}
	</style>
...
case 'register' :
	...
	login_header(__('Registration Form'), '<p class="message register">사용자 등록하기</p>', $errors);
?>
