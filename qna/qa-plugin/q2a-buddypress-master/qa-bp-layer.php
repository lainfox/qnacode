...
<style>',qa_opt('buddypress_integration_css'),'</style>');
		...
		function main_parts($content)
		{
			if (qa_opt('buddypress_integration_enable') && $this->template == 'user' && !qa_get('tab')) {
					...
					$this->output('<div class="profile-section">');
	...
	// handle replacements
		...
		function logged_in()
		{
			if(isset($this->content['loggedin']['data']) && qa_opt('buddypress_integration_enable') && qa_opt('buddypress_display_names')) {
				$userid = qa_get_logged_in_user();				
				$userid = $userid['userid'];				
				$avatar = $this->get_bp_avatar($userid, 16);
				$avatar = '<span class="nav-avatar">'.$avatar.'</span>';
				$this->content['loggedin']['prefix'] = $avatar;
	...
	// worker
		...
		function user_buddypress_form($content) {
			...
			if(qa_opt('buddypress_enable_profile')) {
				...
				$avatar = $this->get_bp_avatar($userid, 150, 'full');
				$avatar = '<span class="user-avatar">'.$avatar.'</span>';
				$this->output( $avatar );
				...
				$title = qa_opt('buddypress_integration_title');
